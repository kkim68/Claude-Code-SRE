...
-----------------------------------------------------------------------------
"import" is ES Module syntax for pulling in code from other files or
installed npm packages.  Under the hood, Node.js resolves the module name,
loads its JavaScript file, executes it once, then returns its exported
values here.  Modules are cached after the first load, so importing the
same module in ten different files does NOT run it ten times.`;

const string2 = `
"express" is the default export from the 'express' npm package.
It is a factory function -- calling express() returns a new Express
application object (conventionally named 'app').

{ Request, Response } are named TypeScript type exports from Express.
They are NOT runtime values -- they exist only during compilation.
  - Request  : describes the shape of an incoming HTTP request object
               (headers, body, query params, etc.)
  - Response : describes the shape of the response object Express gives
               us to send data back to the client
We import them here so TypeScript can type-check our route callbacks.  
`;


const string3 = `
Node.js's built-in "child_process" module lets us spawn OS-level
subprocesses -- essentially running shell commands from within Node.
We use the named export \`exec\` (not the whole module) because that is
the only function needed here.

exec(command, callback) runs \`command\` in a NEW shell (e.g. /bin/sh),
waits for it to finish, then fires callback(err, stdout, stderr).
`;

const string4 = `
"cors" is a third-party npm middleware package.
CORS = Cross-Origin Resource Sharing.
Browsers enforce the "same-origin policy": by default, JavaScript running
on http://localhost:5173 is NOT allowed to fetch data from
http://localhost:3001 because the ports differ (they are different
"origins").  The server must explicitly tell the browser "this origin
is allowed" by sending special HTTP headers like:
  Access-Control-Allow-Origin: http://localhost:5173
The \`cors\` package generates those headers automatically for us.
`;


const string5 = `
app.listen(port, callback) does several things internally:
  1. Calls Node's built-in http.createServer(app) to create an HTTP
     server.  The \`app\` function itself IS the request handler --
     every time an HTTP request arrives, the server calls app(req, res),
     which runs the entire middleware + routing pipeline.
  2. Calls server.listen(PORT) to bind the OS network socket to the
     given TCP port and start accepting incoming TCP connections.
  3. After the socket is successfully bound, fires the callback.

The callback is optional but useful for confirming the server started
successfully, since listen() is also asynchronous -- it does NOT block.

Once listen() is called, Node's event loop stays alive indefinitely,
waiting for incoming connections.  The process will NOT exit on its own
(unlike a script that just runs top-to-bottom and exits).


console.log() writes to standard output (the terminal).
The backtick string is an ES2015 template literal.  \${PORT} is
interpolated at runtime with the actual port value (e.g. 3001).
This message only appears ONCE, when the server first becomes read
`


const string6 = `
This imports the Express Router object exported from routes/auth.ts.
A Router is essentially a mini Express app -- it has its own .get/.post
handlers but is later mounted onto a specific URL prefix via app.use().
Node resolves "./routes/auth" by looking for ./routes/auth.ts (when
using tsx/ts-node) or ./routes/auth.js (after tsc compilation).
`

const string7 = `
Same pattern -- imports the Router defined in routes/watchlist.ts.
All watchlist-related endpoints (add, remove, list) live in that file.
Same pattern -- imports the Router defined in routes/reviews.ts.
All review-related endpoints (create, read, update, delete) live there.  
`

const string8 = `
Calling express() constructs and returns an Express Application object.
Internally, Express creates:
  - A middleware stack (an ordered array of functions to run per request)
  - A route table (a map of HTTP method + path pattern -> handler function)
  - Various configuration settings (view engines, trust proxy, etc.)
Everything we do after this line configures or extends this \`app\` object.
The variable is named \`app\` by convention across virtually all Express
projects worldwide.
`

const string9 = `
process.env is a plain JavaScript object that Node.js populates with the
current shell's environment variables before our code runs.  For example,
if you start the server with \`PORT=8080 node index.js\`, then
process.env.PORT will equal the STRING "8080".

The \`|| 3001\` is a logical OR fallback.  If process.env.PORT is undefined
(no environment variable was set), the expression short-circuits to 3001.

This pattern lets the same codebase run on different ports in different
environments (local dev, staging, production) without changing source code.
In production, hosting platforms like Heroku or Railway inject PORT
automatically.
`

const string10 = `
app.use() adds a function to Express's middleware stack.
Every incoming HTTP request passes through each middleware in the ORDER
they were added with app.use().  Middleware functions have this signature:
  (req, res, next) => void
They can read/modify req and res, then call next() to pass control to the
next middleware, OR send a response directly to terminate the chain.  
`

const string11 = `
app.use() with ONE argument (no path prefix) registers the middleware
for ALL incoming requests regardless of URL.

cors({ origin: "http://localhost:5173" }) creates a middleware function
configured to ONLY allow requests coming from http://localhost:5173
(the Vite dev server that typically serves the React frontend).

What this middleware actually does on each request:
  1. Reads the request's "Origin" header (set automatically by the browser)
  2. If it matches "http://localhost:5173", adds response headers:
       Access-Control-Allow-Origin: http://localhost:5173
       Access-Control-Allow-Methods: GET,HEAD,PUT,PATCH,POST,DELETE
       Vary: Origin
  3. For "preflight" OPTIONS requests (browser's pre-check before a
     POST/PUT/DELETE), it responds immediately with 204 No Content
     and the necessary headers, never reaching our actual route handlers.
  4. Calls next() so the request continues down the middleware stack.

Without this, the browser would silently block all API responses and
your React app would see nothing but network errors in the console.
`

const string12 = `
express.json() is a built-in Express middleware (no separate npm package
needed since Express 4.16).  It parses the raw HTTP request BODY.

When a client sends a POST/PUT request with a JSON payload, the body
arrives as a raw byte stream.  express.json() reads that stream,
calls JSON.parse() on it, and attaches the resulting JavaScript object
to req.body.

Without this middleware, req.body would be \`undefined\` in every route
handler -- your server would have no way to read the data the client sent.

Internally it uses the "body-parser" library and checks the
Content-Type header.  If the header is NOT "application/json", the
middleware skips parsing and calls next() immediately (leaving req.body
as undefined), so it's safe to register globally without harming
non-JSON endpoints.

After this middleware runs, any route handler can safely do:
  const { username, password } = req.body;  
`

const string13 = `
Unlike the routes below that are imported from separate files, this
endpoint is defined directly in index.ts.  This is fine for one-off or
simple endpoints, but larger route groups should live in their own files
under routes/ for maintainability. 
An inline route handler is a callback function defined directly inside a route method call, rather than being declared separately and referenced by name.
The anonymous arrow function (req, res) => { ... } passed directly into app.get() is the inline route handler. There is no separate named function — it lives entirely inside the route definition.
Multiple Inline Handlers (Chained Middleware)Express allows you to pass multiple handlers to a single route. They execute in order, and each must call next() to pass control to the next one.
This pattern is extremely common for inline auth guards or validation steps scoped to a single route without polluting the global middleware chain.
Passing an Array of Handlers. You can also pass handlers as an array, which Express flattens automatically.  
`

const string14 = `
When to Use Inline vs Named Handlers
SituationRecommendationSimple, one-off logicInlineLogic reused across multiple routesNamed functionRoute-specific validation or guardInline (middle position)Complex business logicNamed or controller functionTesting in isolationNamed (easier to unit test)

Key Behavioral Notes

Order matters. Handlers run strictly in the order they are defined.
Termination. Once res.send(), res.json(), or any response method is called, subsequent handlers should not attempt to send another response — Express will throw a "headers already sent" error.
next('route') is a special call that skips the remaining handlers in the current route and moves to the next matching route definition entirely, which is useful in router-level logic.
Async handlers require explicit error forwarding. Wrap async logic with try/catch and pass errors to next(err), or use a wrapper utility like express-async-errors.  
`
...
app.routes()
This method is no longer available. See issue 2049.
...
The following have been deprecated since 4.0 and affect you when upgrading from 3.x to the latest 4.x version. Check the Changelog for the latest changes.
...
whose URL exactly matches "/api/health".

Express compiles all registered routes into a lookup table.  When
a GET /api/health request arrives, Express walks its route table in
registration order, finds this match, and invokes the callback.

The callback receives:
  req (Request)  -- the incoming HTTP request (headers, query, body, etc.)
  res (Response) -- the outgoing HTTP response builder


req.query is an object that Express populates by parsing the URL's
query string.  For a request to /api/health?service=google.com,
req.query would be { service: "google.com" }.

Express types req.query values as string | string[] | ParsedQs | ...
so the cast \`as string\` narrows it to a plain string for TypeScript.

The \`|| "api.themoviedb.org"\` provides a default value if the
caller omits the ?service= parameter entirely.

exec() takes a command string and a callback.
It spawns a NEW /bin/sh process, runs the command inside it, and
calls the callback when the process exits.


\`err\`    -- an Error object if the child process exited with a
            non-zero status code (ping returns 1 when host is
            unreachable), or null on success.
\`stdout\` -- a string containing everything the child process wrote
            to standard output.

Note: the callback runs ASYNCHRONOUSLY.  exec() returns immediately
(it does NOT block Node's event loop), and this inner function is
placed into the event queue and called only after the ping finishes.
This is Node.js's fundamental non-blocking I/O model in action.


res.json() is an Express helper that:
  1. Calls JSON.stringify() on the argument
  2. Sets the Content-Type response header to "application/json"
  3. Sets a default HTTP 200 OK status (if not already set)
  4. Writes the JSON string as the response body and closes the
     connection
After calling res.json() (or any other res.send/end method), you
must NOT attempt to send any more data on this response -- Express
will throw "Cannot set headers after they are sent to the client."

The ternary operator:  condition ? valueIfTrue : valueIfFalse
If exec's callback received an error (ping failed), report
"unreachable".  Otherwise report "healthy".


stdout.match(regex) runs a regular expression against the ping
output string.  Ping outputs something like:
  "64 bytes from X: icmp_seq=1 ttl=64 time=12.3 ms"
The regex /time=(\d+\.?\d*)/ looks for "time=" followed by one
or more digits (\d+), an optional dot (\.?), and more optional
digits (\d*).  The parentheses create a capture group.

match() returns an array: [fullMatch, captureGroup1, ...]
  e.g. ["time=12.3", "12.3"]
or null if no match was found.

The ?. is optional chaining: if match() returned null, ?.[1]
safely evaluates to undefined instead of throwing
"Cannot read properties of null".

The || null converts undefined to null so the JSON output
contains an explicit null rather than omitting the key entirely.  
`
...
function classifyLines(lines) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === '') {
      result.push({ type: 'empty', line });
      continue;
    }

    result.push({ type: 'code', line });
  }
...
if (!targetFile) {
  console.error('Usage: node insert-string.js <target.js>');
  process.exit(1);
}

const content  = fs.readFileSync(targetFile, 'utf8');
const lines    = content.split('\n');
const tagged   = classifyLines(lines);

const output = [];
for (const item of tagged) {
  if (item.type === 'code') {
    output.push(makeComment());  
  }
  output.push(item.line);
}

fs.writeFileSync(targetFile, output.join('\n'), 'utf8');
console.log(`Done: inserted comment blocks into ${targetFile}`);
